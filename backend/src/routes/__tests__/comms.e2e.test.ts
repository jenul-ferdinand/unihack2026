...
describe('Comms E2E — full run lifecycle', () => {
  ...
  it('GET /api/comms/runs — lists the completed run', async () => {
    ...
    expect(body.runs[0]).toHaveProperty('created_at');
    expect(body.runs[0]).toHaveProperty('stopped_at');
  });
  ...
  it('GET /api/comms/runs/:id — returns corrected path', async () => {
    ...
    // Each path point has correct shape
